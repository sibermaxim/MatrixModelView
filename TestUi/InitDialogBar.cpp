...
BOOL CInitDialogBar::Create(LPCTSTR lpszTemplateName, CWnd * pParentWnd, BOOL bHasGripper, LPCTSTR nIDTemplate, UINT nStyle, UINT nID)
{
	if (!CPaneDialog::Create(lpszTemplateName, pParentWnd, bHasGripper, nIDTemplate, nStyle, nID))
...
BOOL CInitDialogBar::Create(CWnd * pParentWnd, UINT nIDTemplate,
	UINT nStyle, UINT nID)
{
	if(!CPaneDialog::Create(pParentWnd, MAKEINTRESOURCE(nIDTemplate), nStyle, nID))
		return FALSE;	// Since there is no WM_INITDIALOG message we have to call
	// our own InitDialog function ourselves after m_hWnd is valid
	if(!OnInitDialogBar())		
		return FALSE;	
